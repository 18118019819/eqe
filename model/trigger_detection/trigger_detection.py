...
class TriggerDetection(nn.Module):
    ...
    def forward(self, dataset_id, sentence_batch, idxs_to_collect_sent, idxs_to_collect_event, embed_lengths, pos_tag):
        ...
        outs = self.bert(sentence_batch, token_type_ids=token_type_ids, attention_mask=attention_mask)
        all_embeddings = outs['last_hidden_state']
        hidden_layer_att = outs['attentions']


        # print(self.bert(sentence_batch,
        #                 token_type_ids=token_type_ids,
        #                 attention_mask=attention_mask))
