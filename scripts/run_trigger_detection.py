...
def main(**kwargs):
    ...
    torch.save(best_model.state_dict(), config.save_model_path + date_time.replace(":", "_"))
    logging.info('Test')
    logging.info('f1_bio: {} |  p:{}  | r:{}'.format(f1, precision, recall))
    save_to_jsonl(json_output, 'outputs/'+date_time.replace(":", "_")+'.json')
    return 0
...
def eval_trigger(model, eval_loader, config, metadata, gold_event):
    ...
    with torch.no_grad():
        for i, batch in enumerate(eval_loader):
            ...
            # this_gold = set(tuple(str(x) if isinstance(x, int) else x for x in gold_event[i]['event_trigger']))
            this_gold = set(tuple(str(x) if isinstance(x, int) else x for x in gold_event[i]['event_trigger']))
...
#     python run_trigger_detection.py --tr_dataset ../data/ace_en/pt/train.pt  --dev_dataset ../data/ace_en/pt/dev.pt
#     python run_trigger_detection.py --train_trigger ../data/ace_en/pt/train.pt  --eval_trigger ../data/ace_en/pt/dev.pt
